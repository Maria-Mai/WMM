...
sms.Z <- function(g, n = length(g)) {
  ...
  w.vec <-
    seq(0, pi, length.out = n) # Vektor mit äquidistanten Kreisfrequenzen
  z.vec <-
    exp(w.vec * 1i) # Vektor mit komplexen Zahlen der Kreisfrequenzen
  mu <-
    seq(1, length(g)) # = mu läuft von 0, ..., M ; M ist erste/letzte Element, das in Frequenzantwort miteinfließt
...
sms.FFT <- function(g, n) {
  l = 2 * (n - 1)
  g.vec <- cbind(g[1:l], rep(0, length(g) - l))
  G <- fft (g.vec)[1:n]
  abs(G) ^ 2
}
